...
split1[3] = split1[3].rstrip()
split2[3] = split2[3].rstrip()

num_haps1=len(split1[3])
num_haps2=len(split2[3])

if len(sys.argv)>5:
    hapstart1, hapend1, hapstart2, hapend2 = int(sys.argv[5]), int(sys.argv[6]), int(sys.argv[7]), int(sys.argv[8])
else:
...
sortfile.write(sort_out.decode("utf-8"))
sortfile.close()
...
sort_pos_lines=sort_pos_out.decode("utf-8").split('\n')
